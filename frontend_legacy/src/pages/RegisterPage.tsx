...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: RegisterForm) => {
    try {
      ...
      toast({ title: "Welcome to IntelVault!", description: "Your account has been created." });
      navigate("/workspaces", { replace: true });
    } catch (err) {
      const message = err instanceof ApiClientError ? String(err.detail) : "Registration failed";
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background p-4">
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute -top-1/2 left-1/2 h-[800px] w-[800px] -translate-x-1/2 rounded-full bg-secondary/5 blur-3xl" />
      </div>
      <motion.div
      ...
      >
        <div className="mb-8 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Start using IntelVault</p>
        </div>

        <form onSubmit={handleSubmit(onSubmit)} className="glass-panel rounded-2xl p-6 space-y-5">
